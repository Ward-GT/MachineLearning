...
class UNet(nn.Module):
    def __init__(self,
                 ...
                 is_attn: List[bool] = (False, False, True, True),
                 n_blocks: int = 1,
                 n_heads: int = 1,
                 dim_head: int = None):
        super().__init__()
        ...
        for i in range(n_resolutions):
            ...
            for _ in range(n_blocks):
                self.down_blocks.append(DownBlock(in_channels, out_channels, n_channels*4, is_attn[i], n_heads, dim_head))
                in_channels = out_channels

            if i < n_resolutions - 1:
                self.down_blocks.append(DownSample(in_channels))

        self.middle = MiddleBlock(out_channels, n_channels*4, n_heads, dim_head)
        ...
        for i in reversed(range(n_resolutions)):
            ...
            for _ in range(n_blocks):
                self.up_blocks.append(UpBlock(in_channels, out_channels, n_channels*4, is_attn[i], n_heads, dim_head))
