...
class AttentionBlock(nn.Module):
    def __init__(self, n_channels: int, n_heads: int = None, dim_head: int = 64, n_groups: int = 32):
        super().__init__()

        if n_heads is None:
            n_heads = n_channels // dim_head
...
class DownBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, time_channels: int, has_attention: bool, n_heads: int = 1, dim_head: int = None):
        super().__init__()

        self.res = ResidualBlock(in_channels, out_channels, time_channels)

        if has_attention:
            self.attn = AttentionBlock(out_channels, n_heads=n_heads, dim_head=dim_head)
        else:
...
class MiddleBlock(nn.Module):
    def __init__(self, n_channels: int, time_channels: int, n_heads: int = 1, dim_head: int = None):
        super().__init__()
        self.res1 = ResidualBlock(n_channels, n_channels, time_channels)
        self.attn = AttentionBlock(n_channels, n_heads=n_heads, dim_head=dim_head)
    ...
    def __init__(
        ...
        for i in range(n_resolutions):
            ...
            for _ in range(n_blocks):
                self.down_blocks.append(DownBlock(in_channels, out_channels, self.time_channels, is_attn[i], n_heads, dim_head))
                in_channels = out_channels

            if i < n_resolutions - 1:
                self.down_blocks.append(DownSample(in_channels))

        self.middle = MiddleBlock(out_channels, self.time_channels, n_heads, dim_head)
        ...
        for i in reversed(range(n_resolutions)):
            ...
            for _ in range(n_blocks):
                self.up_blocks.append(UpBlock(in_channels, out_channels, self.time_channels, is_attn[i], n_heads, dim_head))
